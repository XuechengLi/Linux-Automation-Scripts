...
# 1. 执行 Git Pull
    ...
    #删除当前路径下以往版本
    rm -rf "$deploy/address.houbaoyan.cn/"* || { echo "删除失败" ; exit 1; }
    echo "已删除当前路径$deploy/address.houbaoyan.cn/所有文件"
    echo "-----------------------------------------"
    echo "-----------------------------------------"
    #复制到服务路径下
    cp -r ./dist/* "$deploy/address.houbaoyan.cn/" || { echo "复制失败" ; exit 1; }
    echo "dist包复制到$deploy/address.houbaoyan.cn/"
    echo "-----------------------------------------"
    echo "-----------------------------------------"


    echo "客户发货平台 http://192.168.12.93:8787"